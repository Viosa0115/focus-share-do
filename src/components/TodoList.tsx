...
import { Check, Plus, Trash2, Calendar, RefreshCw, ChevronDown, ChevronUp, Tag, FileText, Trophy, Minus, Play, Square, Flag, Save, X, Clock } from "lucide-react";
...
function TodosSection({ completedTodo, setCompletedTodo }: { completedTodo: any; setCompletedTodo: (v: any) => void }) {
  ...
  // Group active todos by recurrence
  ...
  const getLabel = (labelId: string | null) => {
    ...
    return (labels as any[]).find((l: any) => l.id === labelId);
  };
        ...
        <div className="space-y-4">
          {sections.map(({ key, label, items }) => items.length > 0 && (
            <div key={key} className="space-y-2">
              ...
              {items.map((todo: any) => (
                <TodoItem key={todo.id} todo={todo} label={getLabel(todo.label_id)} onToggle={() => handleToggle(todo)} onDelete={() => deleteTodo.mutate(todo.id)} />
              ))}
            </div>
          ))}
...
function TodoItem({ todo, label, onToggle, onDelete }: { todo: any; label: any; onToggle: () => void; onDelete: () => void }) {
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          ...
          <DialogContent className="rounded-2xl">
            ...
            <form onSubmit={(e) => {
              e.preventDefault();
              createChallenge.mutate({ name, challenge_type: type });
              setName("");
              setShowCreate(false);
            }} className="space-y-4">
          ...
          {(challenges as any[]).map((ch: any) => (
            <div key={ch.id} className="p-4 rounded-2xl bg-card shadow-soft space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => deleteChallenge.mutate(ch.id)} className="text-muted-foreground hover:text-destructive">
                  <Trash2 className="h-4 w-4" />
                </button>
              </div>
              ...
              {ch.challenge_type === "count" && (
                <div className="flex items-center justify-center gap-6 py-2">
                  <button onClick={() => updateChallenge.mutate({ id: ch.id, updates: { score: Math.max(0, (ch.score || 0) - 1) } })}
                  ...
                  <button onClick={() => updateChallenge.mutate({ id: ch.id, updates: { score: (ch.score || 0) + 1 } })}
                    ...
                    {(!timerRunning || timerChallengeId !== ch.id) ? (
                      ...
                      <>
                        <Button size="sm" className="rounded-xl" onClick={() => {
                          ...
                          const best = ch.best_time_ms ? Math.min(ch.best_time_ms, timerElapsed) : timerElapsed;
                          updateChallenge.mutate({ id: ch.id, updates: { best_time_ms: best } });
              ...
              {ch.challenge_type === "endurance" && !ch.given_up && (
                <div className="text-center space-y-3 py-2">
                  ...
                  <Button size="sm" variant="destructive" className="rounded-xl" onClick={() =>
                    updateChallenge.mutate({ id: ch.id, updates: { given_up: true, ended_at: new Date().toISOString() } })
                  }>
              ...
              {ch.challenge_type === "endurance" && ch.given_up && (
                <p className="text-center text-sm text-muted-foreground py-2">
                  Aufgegeben nach {formatDuration(new Date(ch.started_at), new Date(ch.ended_at))}
                </p>
              )}
            </div>
          ))}
